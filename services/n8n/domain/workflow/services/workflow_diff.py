"""
MIGRATION-META:
  source_path: packages/workflow/src/workflow-diff.ts
  target_context: n8n
  target_layer: Domain
  responsibility: 位于 packages/workflow/src 的工作流模块。导入/依赖:外部:lodash/isEqual、lodash/pick；内部:无；本地:.、./connections-diff。导出:WorkflowDiffBase、DiffableNode、DiffableWorkflow、enum、NodeDiff、WorkflowDiff、compareNodes、compareWorkflowsNodes 等7项。关键函数/方法:makeSkipTimeDifference、hasNonPositionalChanges、hasCredentialChanges。用于承载工作流实现细节，并通过导出对外提供能力。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/workflow/src/workflow-diff.ts -> services/n8n/domain/workflow/services/workflow_diff.py
