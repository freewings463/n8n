"""
MIGRATION-META:
  source_path: packages/workflow/src/workflow.ts
  target_context: n8n
  target_layer: Domain
  responsibility: 位于 packages/workflow/src 的工作流模块。导入/依赖:外部:无；内部:@n8n/errors；本地:./errors、./expression、./global-state 等6项。导出:WorkflowParameters、Workflow。关键函数/方法:setNodes、setConnections、setPinData、setSettings、overrideStaticData、getConnectionsByDestination、getStaticData、setTestStaticData、getTriggerNodes、getPollNodes 等18项。用于承载工作流实现细节，并通过导出对外提供能力。注释目标:eslint-disable @typescript-eslint/no-unsafe-member-access。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/workflow/src/workflow.ts -> services/n8n/domain/workflow/services/workflow.py
