"""
MIGRATION-META:
  source_path: packages/workflow/src/expression-sandboxing.ts
  target_context: n8n
  target_layer: Domain
  responsibility: 位于 packages/workflow/src 的工作流模块。导入/依赖:外部:无；内部:@n8n/tournament；本地:./errors、./utils。导出:sanitizerName、DOLLAR_SIGN_ERROR、FunctionThisSanitizer、DollarSignValidator、PrototypeSanitizer、sanitizer。关键函数/方法:isValidDollarPropertyAccess、astVisit、visitCallExpression、visitFunctionExpression、visitIdentifier、visitMemberExpression、visitObjectPattern、sanitizer。用于承载工作流实现细节，并通过导出对外提供能力。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/workflow/src/expression-sandboxing.ts -> services/n8n/domain/workflow/services/expression_sandboxing.py
