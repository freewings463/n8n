"""
MIGRATION-META:
  source_path: packages/workflow/src/run-execution-data-factory.ts
  target_context: n8n
  target_layer: Domain
  responsibility: 位于 packages/workflow/src 的工作流模块。导入/依赖:外部:无；内部:无；本地:./execution-context、./run-execution-data/run-execution-data、./run-execution-data/run-execution-data.v1。导出:CreateFullRunExecutionDataOptions、createRunExecutionData、createEmptyRunExecutionData、createErrorExecutionData。关键函数/方法:createRunExecutionData、createEmptyRunExecutionData、createErrorExecutionData。用于承载工作流实现细节，并通过导出对外提供能力。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/workflow/src/run-execution-data-factory.ts -> services/n8n/domain/workflow/services/run_execution_data_factory.py
