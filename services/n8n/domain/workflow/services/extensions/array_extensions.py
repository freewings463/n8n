"""
MIGRATION-META:
  source_path: packages/workflow/src/extensions/array-extensions.ts
  target_context: n8n
  target_layer: Domain
  responsibility: 位于 packages/workflow/src/extensions 的工作流模块。导入/依赖:外部:lodash/isEqual、lodash/uniqWith；内部:无；本地:./extensions、./object-extensions、../errors/expression-extension.error、../errors/expression.error 等1项。导出:average、toJsonString、toInt、toFloat、toBoolean、toDateTime、arrayExtensions。关键函数/方法:first、isEmpty、isNotEmpty、last、pluck、randomItem、unique、mapForEqualityCheck、ensureNumberArray、sum 等18项。用于承载工作流实现细节，并通过导出对外提供能力。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/workflow/src/extensions/array-extensions.ts -> services/n8n/domain/workflow/services/extensions/array_extensions.py
