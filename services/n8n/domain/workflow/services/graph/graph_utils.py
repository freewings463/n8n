"""
MIGRATION-META:
  source_path: packages/workflow/src/graph/graph-utils.ts
  target_context: n8n
  target_layer: Domain
  responsibility: 位于 packages/workflow/src/graph 的工作流模块。导入/依赖:外部:无；内部:无；本地:../interfaces。导出:ExtractableErrorResult、IConnectionAdjacencyList、getInputEdges、getOutputEdges、getRootNodes、getLeafNodes、hasPath、ExtractableSubgraphData 等2项。关键函数/方法:getInputEdges、getOutputEdges、getRootNodes、getLeafNodes、intersection、hasPath、buildAdjacencyList、addOrCreate、parseInt、parseExtractableSubgraphSelection。用于承载工作流实现细节，并通过导出对外提供能力。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/workflow/src/graph/graph-utils.ts -> services/n8n/domain/workflow/services/graph/graph_utils.py
