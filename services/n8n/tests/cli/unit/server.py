"""
MIGRATION-META:
  source_path: packages/cli/src/server.ts
  target_context: n8n
  target_layer: Infrastructure
  responsibility: 位于 packages/cli/src 的入口。导入/依赖:外部:cookie-parser、express 等3项；内部:@n8n/backend-common、@n8n/config、@n8n/constants、@n8n/db 等70项；本地:./chat/chat-server、./mfa/mfa.service、../pubsub/pubsub.registry。导出:Server。关键函数/方法:start、registerAdditionalControllers、configure、next、async、isApiEnabled、securityHeadersMiddleware、setCustomCacheHeader、installGlobalProxyAgent、configureSettingsRoute 等2项。用于汇总导出并完成该模块模块初始化、注册或装配。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/cli/src/server.ts -> services/n8n/tests/cli/unit/server.py
