"""
MIGRATION-META:
  source_path: packages/testing/containers/services/tracing.ts
  target_context: n8n
  target_layer: Infrastructure
  responsibility: 位于 packages/testing/containers/services 的服务。导入/依赖:外部:testcontainers；内部:无；本地:../test-containers、./types。导出:TracingConfig、TracingMeta、TracingResult、TracerWebhookConfig、tracing、TracingHelper、createTracingHelper。关键函数/方法:start、env、getWebhookConfig、createTracingHelper。用于封装该模块业务流程，对上提供稳定调用面。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/testing/containers/services/tracing.ts -> services/n8n/tests/testing/fixtures/containers/services/tracing.py
