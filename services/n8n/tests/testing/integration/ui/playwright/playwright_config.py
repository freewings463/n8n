"""
MIGRATION-META:
  source_path: packages/testing/playwright/playwright.config.ts
  target_context: n8n
  target_layer: Infrastructure
  responsibility: 位于 packages/testing/playwright 的配置。导入/依赖:外部:@currents/playwright、@playwright/test；内部:无；本地:./currents.config、./playwright-projects、./utils/url-helper。导出:无。关键函数/方法:getTestEnv。用于集中定义该模块配置项与默认值，供其他模块读取。注释目标:eslint-disable import-x/no-default-export。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/testing/playwright/playwright.config.ts -> services/n8n/tests/testing/integration/ui/playwright/playwright_config.py
