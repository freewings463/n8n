"""
MIGRATION-META:
  source_path: packages/@n8n/nodes-langchain/utils/agent-execution/buildSteps.ts
  target_context: n8n
  target_layer: Infrastructure
  responsibility: 位于 packages/@n8n/nodes-langchain/utils/agent-execution 的执行工具。导入/依赖:外部:@langchain/core/messages；内部:n8n-workflow；本地:无。导出:buildSteps。关键函数/方法:extractProviderMetadata、buildAnthropicContentBlocks、buildMessageContent、buildSteps。用于提供执行通用工具能力（纯函数/封装器）供复用。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/@n8n/nodes-langchain/utils/agent-execution/buildSteps.ts -> services/n8n/infrastructure/n8n-nodes-langchain/external_services/utils/agent-execution/buildSteps.py
