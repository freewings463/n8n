"""
MIGRATION-META:
  source_path: packages/nodes-base/scripts/validate-schema-versions.js
  target_context: n8n
  target_layer: Infrastructure
  responsibility: 位于 packages/nodes-base/scripts 的模块。导入/依赖:外部:无；内部:无；本地:无。导出:无。关键函数/方法:findSchemaDirectories、getAvailableSchemaVersions、findNodeFile、extractVersionFromSource、versionToSchemaFormat、validateSchemaVersions。用于承载该模块实现细节，并通过导出对外提供能力。注释目标:Validates that schema files exist for all declared node versions. / This script checks nodes that have __schema__ directories and verifies。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/nodes-base/scripts/validate-schema-versions.js -> services/n8n/infrastructure/nodes-base/container/scripts/validate_schema_versions.py
