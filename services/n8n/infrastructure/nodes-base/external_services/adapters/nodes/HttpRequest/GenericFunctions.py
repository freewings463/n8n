"""
MIGRATION-META:
  source_path: packages/nodes-base/nodes/HttpRequest/GenericFunctions.ts
  target_context: n8n
  target_layer: Infrastructure
  responsibility: 位于 packages/nodes-base/nodes/HttpRequest 的节点。导入/依赖:外部:form-data、lodash/get 等3项；内部:无；本地:./interfaces 等1项。导出:BodyParameter、IAuthDataSanitizeKeys、replaceNullValues、REDACTED、sanitizeUiMessage、getSecrets、getOAuth2AdditionalParameters、binaryContentTypes 等4项。关键函数/方法:set、replaceNullValues、isObject、setSafeObjectProperty、sanitizeUiMessage、getSecrets 等4项。用于实现 n8n 该模块节点的描述与执行逻辑，供工作流运行。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/nodes-base/nodes/HttpRequest/GenericFunctions.ts -> services/n8n/infrastructure/nodes-base/external_services/adapters/nodes/HttpRequest/GenericFunctions.py
