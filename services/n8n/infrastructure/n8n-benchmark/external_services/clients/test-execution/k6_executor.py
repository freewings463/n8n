"""
MIGRATION-META:
  source_path: packages/@n8n/benchmark/src/test-execution/k6-executor.ts
  target_context: n8n
  target_layer: Infrastructure
  responsibility: 位于 packages/@n8n/benchmark/src/test-execution 的执行模块。导入/依赖:外部:node:assert/strict、zx、https:/…/index.js；内部:@/test-execution/app-metrics-poller、@/test-execution/test-report、@/types/scenario；本地:无。导出:K6ExecutorOpts、K6RunOpts、K6Executor、handleSummary。关键函数/方法:handleSummary、executeTestScenario、sendTestReport、assert、augmentSummaryScript、loadEndOfTestSummary、resolveK6ExecutablePath。用于承载执行实现细节，并通过导出对外提供能力。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/@n8n/benchmark/src/test-execution/k6-executor.ts -> services/n8n/infrastructure/n8n-benchmark/external_services/clients/test-execution/k6_executor.py
