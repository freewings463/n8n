"""
MIGRATION-META:
  source_path: packages/cli/src/modules/insights/insights-collection.service.ts
  target_context: n8n
  target_layer: Infrastructure
  responsibility: 位于 packages/cli/src/modules/insights 的Insights服务。导入/依赖:外部:luxon；内部:@n8n/backend-common、@n8n/db 等5项；本地:../repositories/insights-metadata.repository 等2项。导出:InsightsCollectionService。关键函数/方法:init、scheduleFlushing、async、cancelScheduledFlushing、clearTimeout、stopFlushingTimer、shutdown、handleWorkflowExecuteAfter、saveInsightsMetadataAndRaw、flushEvents 等2项。用于封装Insights业务流程，对上提供稳定调用面。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/cli/src/modules/insights/insights-collection.service.ts -> services/n8n/infrastructure/cli/persistence/repositories/modules/insights/insights_collection_service.py
