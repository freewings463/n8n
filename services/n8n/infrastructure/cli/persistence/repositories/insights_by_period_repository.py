"""
MIGRATION-META:
  source_path: packages/cli/src/modules/insights/database/repositories/insights-by-period.repository.ts
  target_context: n8n
  target_layer: Infrastructure
  responsibility: 位于 packages/cli/src/modules/insights/database 的Insights仓储。导入/依赖:外部:luxon、zod；内部:@n8n/config、@n8n/db 等2项；本地:./insights-by-period-query.helper 等2项。导出:InsightsByPeriodRepository。关键函数/方法:escapeField、getPeriodFilterExpr、getPeriodStartExpr、getPeriodInsightsBatchQuery、getAggregationQuery、compactSourceDataIntoInsightPeriod、getPreviousAndCurrentPeriodTypeAggregates 等5项。用于封装Insights数据读写与查询细节，隔离持久层。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/cli/src/modules/insights/database/repositories/insights-by-period.repository.ts -> services/n8n/infrastructure/cli/persistence/repositories/insights_by_period_repository.py
