"""
MIGRATION-META:
  source_path: packages/@n8n/task-runner/src/js-task-runner/built-ins-parser/built-ins-parser.ts
  target_context: n8n
  target_layer: Infrastructure
  responsibility: 位于 packages/@n8n/task-runner/src/js-task-runner/built-ins-parser 的模块。导入/依赖:外部:acorn、acorn-walk；内部:n8n-workflow；本地:./built-ins-parser-state。导出:BuiltInsParser。关键函数/方法:parseUsedBuiltIns、identifyBuiltInsByWalkingAst、ancestor、visitDollarCallExpression、visitDollarItemsCallExpression、handlePrevNodeCall、isPairedItemProperty。用于承载该模块实现细节，并通过导出对外提供能力。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/@n8n/task-runner/src/js-task-runner/built-ins-parser/built-ins-parser.ts -> services/n8n/infrastructure/n8n-task-runner/container/js-task-runner/built-ins-parser/built_ins_parser.py
