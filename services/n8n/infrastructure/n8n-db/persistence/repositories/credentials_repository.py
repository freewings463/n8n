"""
MIGRATION-META:
  source_path: packages/@n8n/db/src/repositories/credentials.repository.ts
  target_context: n8n
  target_layer: Infrastructure
  responsibility: 位于 packages/@n8n/db/src/repositories 的仓储。导入/依赖:外部:无；内部:@n8n/di、@n8n/typeorm；本地:../entities、../entities/types-db。导出:CredentialsRepository。关键函数/方法:findStartingWith、findMany、toFindManyOptions、handleSharedFilters、getManyByIds、findAllGlobalCredentials、findAllPersonalCredentials、findAllCredentialsForWorkflow、findAllCredentialsForProject。用于封装该模块数据读写与查询细节，隔离持久层。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/@n8n/db/src/repositories/credentials.repository.ts -> services/n8n/infrastructure/n8n-db/persistence/repositories/credentials_repository.py
