"""
MIGRATION-META:
  source_path: packages/@n8n/db/src/migrations/dsl/table.ts
  target_context: n8n
  target_layer: Infrastructure
  responsibility: 位于 packages/@n8n/db/src/migrations/dsl 的迁移。导入/依赖:外部:p-lazy；内部:@n8n/typeorm、n8n-workflow；本地:./column。导出:CreateTable、DropTable、AddColumns、DropColumns、AddForeignKey、DropForeignKey、AddNotNull、DropNotNull。关键函数/方法:execute、withColumns、withIndexOn、withUniqueConstraintOn、withCheck、withEnumCheck、withForeignKey。用于定义该模块数据库迁移步骤（新增表/字段/索引等）。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/@n8n/db/src/migrations/dsl/table.ts -> services/n8n/infrastructure/n8n-db/persistence/migrations/dsl/table.py
