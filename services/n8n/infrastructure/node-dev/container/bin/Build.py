"""
MIGRATION-META:
  source_path: packages/node-dev/src/Build.ts
  target_context: n8n
  target_layer: Infrastructure
  responsibility: 位于 packages/node-dev/src 的模块。导入/依赖:外部:fast-glob、fs/promises、tmp-promise；内部:@n8n/di、n8n-core、n8n-workflow；本地:./Interfaces。导出:无。关键函数/方法:createCustomTsconfig、buildFiles。用于承载该模块实现细节，并通过导出对外提供能力。注释目标:eslint-disable @typescript-eslint/no-unsafe-argument。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/node-dev/src/Build.ts -> services/n8n/infrastructure/node-dev/container/bin/Build.py
