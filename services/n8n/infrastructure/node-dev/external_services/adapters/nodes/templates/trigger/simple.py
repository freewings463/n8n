"""
MIGRATION-META:
  source_path: packages/node-dev/templates/trigger/simple.ts
  target_context: n8n
  target_layer: Infrastructure
  responsibility: 位于 packages/node-dev/templates/trigger 的模块。导入/依赖:外部:无；内部:n8n-workflow；本地:无。导出:ClassNameReplace。关键函数/方法:trigger、executeTrigger、closeFunction、clearInterval、manualTriggerFunction。用于承载该模块实现细节，并通过导出对外提供能力。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/node-dev/templates/trigger/simple.ts -> services/n8n/infrastructure/node-dev/external_services/adapters/nodes/templates/trigger/simple.py
