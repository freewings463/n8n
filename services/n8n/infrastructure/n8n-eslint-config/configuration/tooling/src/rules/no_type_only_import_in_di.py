"""
MIGRATION-META:
  source_path: packages/@n8n/eslint-config/src/rules/no-type-only-import-in-di.ts
  target_context: n8n
  target_layer: Infrastructure
  responsibility: 位于 packages/@n8n/eslint-config/src/rules 的模块。导入/依赖:外部:@typescript-eslint/utils；内部:无；本地:无。导出:NoTypeOnlyImportInDiRule。关键函数/方法:create、ImportDeclaration、ClassDeclaration、fix。用于承载该模块实现细节，并通过导出对外提供能力。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/@n8n/eslint-config/src/rules/no-type-only-import-in-di.ts -> services/n8n/infrastructure/n8n-eslint-config/configuration/tooling/src/rules/no_type_only_import_in_di.py
