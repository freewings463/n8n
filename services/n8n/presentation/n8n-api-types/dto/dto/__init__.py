"""
MIGRATION-META:
  source_path: packages/@n8n/api-types/src/dto/index.ts
  target_context: n8n
  target_layer: Interface
  responsibility: 位于 packages/@n8n/api-types/src/dto 的入口。导入/依赖:外部:无；内部:无；本地:无。导出:AiAskRequestDto、AiChatRequestDto、AiBuilderChatRequestDto、AiApplySuggestionRequestDto、AiFreeCreditsRequestDto、AiSessionRetrievalRequestDto、AiSessionMetadataResponseDto、AiTruncateMessagesRequestDto 等79项。关键函数/方法:无。用于汇总导出并完成该模块模块初始化、注册或装配。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/@n8n/api-types/src/dto/index.ts -> services/n8n/presentation/n8n-api-types/dto/dto/__init__.py
