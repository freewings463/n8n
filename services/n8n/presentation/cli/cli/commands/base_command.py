"""
MIGRATION-META:
  source_path: packages/cli/src/commands/base-command.ts
  target_context: n8n
  target_layer: Interface
  responsibility: 位于 packages/cli/src/commands 的模块。导入/依赖:外部:reflect-metadata；内部:@n8n/config、@n8n/constants、@n8n/db、@n8n/di、n8n-workflow、@/abstract-server 等17项；本地:无。导出:无。关键函数/方法:init、async、stopProcess、initCrashJournal、exitSuccessFully、exitWithCrash、log、error、initBinaryDataService、initDataDeduplicationService 等8项。用于承载该模块实现细节，并通过导出对外提供能力。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/cli/src/commands/base-command.ts -> services/n8n/presentation/cli/cli/commands/base_command.py
