"""
MIGRATION-META:
  source_path: packages/cli/src/controller.registry.ts
  target_context: n8n
  target_layer: Interface
  responsibility: 位于 packages/cli/src 的控制器。导入/依赖:外部:express、express-rate-limit 等1项；内部:@/constants、@n8n/backend-common、@n8n/config 等10项；本地:../response-errors/not-found.error、./services/last-active-at.service。导出:ControllerRegistry。关键函数/方法:activate、activateController、staticRouters、handler、buildMiddlewares、createRateLimitMiddleware、createLicenseMiddleware、next、createScopedMiddleware。用于处理该模块接口请求，调度服务/仓储并返回响应。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/cli/src/controller.registry.ts -> services/n8n/presentation/cli/api/middleware/controller_registry.py
