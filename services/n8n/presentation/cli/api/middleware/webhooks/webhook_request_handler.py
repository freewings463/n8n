"""
MIGRATION-META:
  source_path: packages/cli/src/webhooks/webhook-request-handler.ts
  target_context: n8n
  target_layer: Interface
  responsibility: 位于 packages/cli/src/webhooks 的Webhook模块。导入/依赖:外部:express、stream/promises；内部:@n8n/backend-common、@n8n/di、n8n-workflow、@/errors/…/webhook-not-found.error 等1项；本地:无。导出:createWebhookHandlerFor。关键函数/方法:handleRequest、sendWebhookResponse、sendStreamResponse、sendStaticResponse、setResponseStatus、setResponseHeaders、sendLegacyResponse、setupCorsHeaders、createWebhookHandlerFor。用于承载Webhook实现细节，并通过导出对外提供能力。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/cli/src/webhooks/webhook-request-handler.ts -> services/n8n/presentation/cli/api/middleware/webhooks/webhook_request_handler.py
