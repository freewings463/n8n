"""
MIGRATION-META:
  source_path: packages/cli/src/task-runners/task-broker/task-broker-server.ts
  target_context: n8n
  target_layer: Interface
  responsibility: 位于 packages/cli/src/task-runners/task-broker 的模块。导入/依赖:外部:compression、express 等5项；内部:@n8n/backend-common、@n8n/config、@n8n/di、@/middlewares、@/response-helper 等2项；本地:无。导出:TaskBrokerServer。关键函数/方法:start、stop、stopHttpServerTask、setupHttpServer、setupWsServer、setupErrorHandlers、setupExpressErrorHandler、setupCommonMiddlewares、configureRoutes、createRateLimiter 等3项。用于承载该模块实现细节，并通过导出对外提供能力。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/cli/src/task-runners/task-broker/task-broker-server.ts -> services/n8n/presentation/cli/api/task-runners/task-broker/task_broker_server.py
