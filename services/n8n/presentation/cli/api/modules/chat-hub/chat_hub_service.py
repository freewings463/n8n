"""
MIGRATION-META:
  source_path: packages/cli/src/modules/chat-hub/chat-hub.service.ts
  target_context: n8n
  target_layer: Interface
  responsibility: 位于 packages/cli/src/modules/chat-hub 的服务。导入/依赖:外部:express、uuid；内部:@n8n/backend-common、@n8n/config 等11项；本地:./chat-hub-agent.service 等11项。导出:ChatHubService。关键函数/方法:deleteChatWorkflow、getErrorMessage、getAIOutput、pickCredentialId、sendHumanMessage、editMessage、regenerateAIMessage、prepareReplyWorkflow、prepareBaseChatWorkflow、prepareChatAgentWorkflow 等5项。用于封装该模块业务流程，对上提供稳定调用面。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/cli/src/modules/chat-hub/chat-hub.service.ts -> services/n8n/presentation/cli/api/modules/chat-hub/chat_hub_service.py
