"""
MIGRATION-META:
  source_path: packages/cli/src/workflows/workflows.controller.ts
  target_context: n8n
  target_layer: Interface
  responsibility: 位于 packages/cli/src/workflows 的工作流控制器。导入/依赖:外部:axios、express 等1项；内部:@n8n/backend-common、@n8n/config 等24项；本地:../collaboration/collaboration.service、./workflow-execution.service、./workflow-finder.service、./workflow-history/workflow-history.service 等4项。导出:WorkflowsController。关键函数/方法:create、update、delete、getAll、getNewName、getFromUrl、getWorkflow、exists、getWriteLock、archive 等8项。用于处理工作流接口请求，调度服务/仓储并返回响应。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/cli/src/workflows/workflows.controller.ts -> services/n8n/presentation/cli/api/workflows/workflows_controller.py
