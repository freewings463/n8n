"""
MIGRATION-META:
  source_path: packages/cli/src/sso.ee/saml/routes/saml.controller.ee.ts
  target_context: n8n
  target_layer: Interface
  responsibility: 位于 packages/cli/src/sso.ee/saml/routes 的SSO控制器。导入/依赖:外部:express、querystring 等1项；内部:@n8n/api-types、@n8n/db、@n8n/decorators 等6项；本地:../saml-helpers、../saml.service.ee、../types、../views/init-sso-post。导出:SamlController。关键函数/方法:getServiceProviderMetadata、configGet、configPost、toggleEnabledPost、acsGet、acsPost、acsHandler、initSsoGet、configTestGet、handleInitSSO 等1项。用于处理SSO接口请求，调度服务/仓储并返回响应。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/cli/src/sso.ee/saml/routes/saml.controller.ee.ts -> services/n8n/presentation/cli/api/sso.ee/saml/routes/saml_controller_ee.py
