"""
MIGRATION-META:
  source_path: packages/cli/src/evaluation.ee/test-runs.controller.ee.ts
  target_context: n8n
  target_layer: Interface
  responsibility: 位于 packages/cli/src/evaluation.ee 的控制器。导入/依赖:外部:express；内部:@n8n/db、@n8n/decorators、n8n-core、n8n-workflow、@/errors/…/conflict.error、@/errors/…/not-found.error 等7项；本地:无。导出:TestRunsController。关键函数/方法:delete、create、getTestRun、getMany、getOne、getTestCases、cancel。用于处理该模块接口请求，调度服务/仓储并返回响应。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/cli/src/evaluation.ee/test-runs.controller.ee.ts -> services/n8n/presentation/cli/api/evaluation.ee/test_runs_controller_ee.py
