"""
MIGRATION-META:
  source_path: packages/cli/src/modules/mcp/tools/webhook-utils.ts
  target_context: n8n
  target_layer: Application
  responsibility: 位于 packages/cli/src/modules/mcp/tools 的Webhook模块。导入/依赖:外部:无；内部:@n8n/db、@/credentials/credentials.service；本地:../mcp.types。导出:WebhookEndpoints、buildWebhookPath、getTriggerDetails、getWebhookDetails。关键函数/方法:buildWebhookPath、getTriggerDetails、getScheduleTriggerDetails、getFormTriggerDetails、getChatTriggerDetails、getWebhookDetails、async、collectWebhookNodeDetails 等7项。用于承载Webhook实现细节，并通过导出对外提供能力。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/cli/src/modules/mcp/tools/webhook-utils.ts -> services/n8n/application/cli/services/modules/mcp/tools/webhook_utils.py
