"""
MIGRATION-META:
  source_path: packages/cli/src/executions/execution.service.ts
  target_context: n8n
  target_layer: Application
  responsibility: 位于 packages/cli/src/executions 的执行服务。导入/依赖:外部:jsonschema；内部:@n8n/backend-common、@n8n/config 等15项；本地:./execution.types。导出:schemaGetExecutionsQueryFilter、allowedExecutionsQueryFilterFields、ExecutionService。关键函数/方法:delete、stop、findOne、getLastSuccessfulExecution、retry、workflowData、createErrorExecution、findRangeWithCount、findLatestCurrentAndCompleted 等10项。用于封装执行业务流程，对上提供稳定调用面。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/cli/src/executions/execution.service.ts -> services/n8n/application/cli/services/executions/execution_service.py
