"""
MIGRATION-META:
  source_path: packages/cli/src/executions/pre-execution-checks/subworkflow-policy-checker.ts
  target_context: n8n
  target_layer: Application
  responsibility: 位于 packages/cli/src/executions/pre-execution-checks 的工作流模块。导入/依赖:外部:无；内部:@n8n/backend-common、@n8n/config、@n8n/db、@n8n/di、n8n-workflow、@/errors/subworkflow-policy-denial.error 等3项；本地:无。导出:SubworkflowPolicyChecker。关键函数/方法:check、errorDetails、findPolicy、findProjects、isListed、logDenial。用于承载工作流实现细节，并通过导出对外提供能力。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/cli/src/executions/pre-execution-checks/subworkflow-policy-checker.ts -> services/n8n/application/cli/services/executions/pre-execution-checks/subworkflow_policy_checker.py
