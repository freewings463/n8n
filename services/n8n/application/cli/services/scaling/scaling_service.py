"""
MIGRATION-META:
  source_path: packages/cli/src/scaling/scaling.service.ts
  target_context: n8n
  target_layer: Application
  responsibility: 位于 packages/cli/src/scaling 的服务。导入/依赖:外部:node:assert、bull；内部:@n8n/backend-common、@n8n/config、@n8n/constants、@n8n/db、@n8n/decorators、@n8n/di 等7项；本地:./constants、./job-processor。导出:ScalingService。关键函数/方法:stop、setupQueue、setupWorker、reportJobProcessingError、pauseQueue、stopMain、stopWorker、pingQueue、getPendingJobCounts、addJob 等23项。用于封装该模块业务流程，对上提供稳定调用面。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/cli/src/scaling/scaling.service.ts -> services/n8n/application/cli/services/scaling/scaling_service.py
