"""
MIGRATION-META:
  source_path: packages/cli/src/task-runners/task-runner-module.ts
  target_context: n8n
  target_layer: Application
  responsibility: 位于 packages/cli/src/task-runners 的模块。导入/依赖:外部:node:assert/strict；内部:@n8n/backend-common、@n8n/config 等12项；本地:./errors/missing-auth-token.error、./errors/missing-requirements.error 等3项。导出:TaskRunnerModule。关键函数/方法:start、stop、stopRunnerProcessTask、stopPythonRunnerProcessTask、stopRunnerServerTask、loadTaskRequester、loadTaskBroker、startInternalTaskRunners。用于承载该模块实现细节，并通过导出对外提供能力。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/cli/src/task-runners/task-runner-module.ts -> services/n8n/application/cli/services/task-runners/task_runner_module.py
