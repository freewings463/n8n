"""
MIGRATION-META:
  source_path: packages/cli/src/task-runners/task-runner-process-base.ts
  target_context: n8n
  target_layer: Application
  responsibility: 位于 packages/cli/src/task-runners 的模块。导入/依赖:外部:node:assert/strict、node:child_process；内部:@n8n/backend-common、@n8n/config 等3项；本地:./forward-to-logger、../auth/task-broker-auth.service、./task-runner-lifecycle-events。导出:ChildProcess、ExitReason、TaskRunnerProcessEventMap。关键函数/方法:start、stop、assert、forwardToLogger、forceRestart、onProcessExit、resolveFn、setImmediate、monitorProcess。用于承载该模块实现细节，并通过导出对外提供能力。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/cli/src/task-runners/task-runner-process-base.ts -> services/n8n/application/cli/services/task-runners/task_runner_process_base.py
