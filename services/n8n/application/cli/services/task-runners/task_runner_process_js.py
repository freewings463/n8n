"""
MIGRATION-META:
  source_path: packages/cli/src/task-runners/task-runner-process-js.ts
  target_context: n8n
  target_layer: Application
  responsibility: 位于 packages/cli/src/task-runners 的模块。导入/依赖:外部:node:assert/strict、node:child_process、node:process；内部:@n8n/backend-common、@n8n/config、@n8n/di；本地:./node-process-oom-detector、../auth/task-broker-auth.service、./task-runner-lifecycle-events、./task-runner-process-base。导出:JsTaskRunnerProcess。关键函数/方法:assert、startProcess、setupProcessMonitoring、analyzeExitReason、getProcessEnvVars。用于承载该模块实现细节，并通过导出对外提供能力。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/cli/src/task-runners/task-runner-process-js.ts -> services/n8n/application/cli/services/task-runners/task_runner_process_js.py
