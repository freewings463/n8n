"""
MIGRATION-META:
  source_path: packages/cli/src/user-management/email/user-management-mailer.ts
  target_context: n8n
  target_layer: Application
  responsibility: 位于 packages/cli/src/user-management/email 的模块。导入/依赖:外部:fs/promises、handlebars；内部:@n8n/backend-common、@n8n/config、@n8n/db、@n8n/di、@n8n/permissions、n8n-workflow 等5项；本地:./interfaces、./node-mailer。导出:UserManagementMailer。关键函数/方法:invite、passwordReset、notifyWorkflowAutodeactivated、notifyWorkflowShared、notifyCredentialsShared、notifyProjectShared、getTemplate。用于承载该模块实现细节，并通过导出对外提供能力。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/cli/src/user-management/email/user-management-mailer.ts -> services/n8n/application/cli/services/user-management/email/user_management_mailer.py
