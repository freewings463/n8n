"""
MIGRATION-META:
  source_path: packages/cli/src/active-executions.ts
  target_context: n8n
  target_layer: Application
  responsibility: 位于 packages/cli/src 的执行模块。导入/依赖:外部:node:assert、p-cancelable；内部:@n8n/backend-common、@n8n/config、@n8n/db、@n8n/di 等4项；本地:./concurrency/concurrency-control.service、./events/event.service。导出:ActiveExecutions。关键函数/方法:has、add、assert、attachWorkflowExecution、attachResponsePromise、resolveResponsePromise、sendChunk、stopExecution、finalizeExecution、resolveExecutionResponsePromise 等6项。用于承载执行实现细节，并通过导出对外提供能力。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/cli/src/active-executions.ts -> services/n8n/application/cli/services/active_executions.py
