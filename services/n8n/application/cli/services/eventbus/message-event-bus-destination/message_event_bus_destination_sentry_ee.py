"""
MIGRATION-META:
  source_path: packages/cli/src/eventbus/message-event-bus-destination/message-event-bus-destination-sentry.ee.ts
  target_context: n8n
  target_layer: Application
  responsibility: 位于 packages/cli/src/eventbus/message-event-bus-destination 的模块。导入/依赖:外部:@sentry/node；内部:n8n-workflow、@/constants；本地:./message-event-bus-destination.ee 等2项。导出:isMessageEventBusDestinationSentryOptions、MessageEventBusDestinationSentry。关键函数/方法:isMessageEventBusDestinationSentryOptions、receiveFromEventBus、level、confirmCallback 等4项。用于承载该模块实现细节，并通过导出对外提供能力。注释目标:eslint-disable @typescript-eslint/no-unsa…
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/cli/src/eventbus/message-event-bus-destination/message-event-bus-destination-sentry.ee.ts -> services/n8n/application/cli/services/eventbus/message-event-bus-destination/message_event_bus_destination_sentry_ee.py
