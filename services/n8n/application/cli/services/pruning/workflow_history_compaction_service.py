"""
MIGRATION-META:
  source_path: packages/cli/src/services/pruning/workflow-history-compaction.service.ts
  target_context: n8n
  target_layer: Application
  responsibility: 位于 packages/cli/src/services/pruning 的工作流服务。导入/依赖:外部:node:assert；内部:@n8n/backend-common、@n8n/config、@n8n/constants、@n8n/db、@n8n/decorators、@n8n/di 等2项；本地:无。导出:WorkflowHistoryCompactionService。关键函数/方法:init、strict、startCompacting、stopCompacting、clearInterval、scheduleRollingCompacting、rateMs、shutdown、compactRecentHistories、compactHistories。用于封装工作流业务流程，对上提供稳定调用面。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/cli/src/services/pruning/workflow-history-compaction.service.ts -> services/n8n/application/cli/services/pruning/workflow_history_compaction_service.py
