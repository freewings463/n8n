"""
MIGRATION-META:
  source_path: packages/cli/src/services/pruning/executions-pruning.service.ts
  target_context: n8n
  target_layer: Application
  responsibility: 位于 packages/cli/src/services/pruning 的执行服务。导入/依赖:外部:node:assert；内部:@n8n/backend-common、@n8n/config、@n8n/constants、@n8n/db、@n8n/decorators、@n8n/di 等2项；本地:无。导出:ExecutionsPruningService。关键函数/方法:init、strict、startPruning、stopPruning、clearInterval、clearTimeout、scheduleRollingSoftDeletions、async、scheduleNextHardDeletion、softDelete 等2项。用于封装执行业务流程，对上提供稳定调用面。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/cli/src/services/pruning/executions-pruning.service.ts -> services/n8n/application/cli/services/pruning/executions_pruning_service.py
