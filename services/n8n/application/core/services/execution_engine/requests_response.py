"""
MIGRATION-META:
  source_path: packages/core/src/execution-engine/requests-response.ts
  target_context: n8n
  target_layer: Application
  responsibility: 位于 packages/core/src/execution-engine 的执行模块。导入/依赖:外部:无；内部:@n8n/di；本地:../errors/error-reporter。导出:handleRequest、isEngineRequest、makeEngineResponse。关键函数/方法:prepareRequestedNodesForExecution、itemIndex、prepareRequestingNodeForResuming、handleRequest、isEngineRequest、makeEngineResponse。用于承载执行实现细节，并通过导出对外提供能力。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/core/src/execution-engine/requests-response.ts -> services/n8n/application/core/services/execution_engine/requests_response.py
