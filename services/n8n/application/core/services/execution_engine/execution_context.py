"""
MIGRATION-META:
  source_path: packages/core/src/execution-engine/execution-context.ts
  target_context: n8n
  target_layer: Application
  responsibility: 位于 packages/core/src/execution-engine 的执行模块。导入/依赖:外部:无；内部:@n8n/backend-common、@n8n/di、@/utils/assertions；本地:./execution-context.service。导出:establishExecutionContext。关键函数/方法:establishExecutionContext、assertExecutionDataExists。用于承载执行实现细节，并通过导出对外提供能力。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/core/src/execution-engine/execution-context.ts -> services/n8n/application/core/services/execution_engine/execution_context.py
