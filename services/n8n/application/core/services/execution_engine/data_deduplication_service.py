"""
MIGRATION-META:
  source_path: packages/core/src/data-deduplication-service.ts
  target_context: n8n
  target_layer: Application
  responsibility: 位于 packages/core/src 的服务。导入/依赖:外部:lodash/get、node:assert/strict；内部:无；本地:无。导出:DataDeduplicationService。关键函数/方法:init、assertDeduplicator、assertInstance、assertSingleInstance、getInstance、checkProcessedItemsAndRecord、checkProcessedAndRecord、removeProcessed、clearAllProcessedItems、getProcessedDataCount。用于封装该模块业务流程，对上提供稳定调用面。
  entities: []
...
"""
# TODO-REFACTOR-DDD: packages/core/src/data-deduplication-service.ts -> services/n8n/application/core/services/execution_engine/data_deduplication_service.py
